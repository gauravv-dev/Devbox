...
/// Legacy digest algorithms CryptoKit doesn't ship (MD5, SHA-1).
/// Shared by the Hashes tool and UUID name-based generation (v3/v5).
/// Verified against RFC 1321 / FIPS 180-1 test vectors and system `md5`/`shasum`.
enum LegacyDigest {
    // MARK: MD5 (RFC 1321)
    ...
    private static let md5Shift: [UInt32] = [
    ...
    static func md5(of input: [UInt8]) -> [UInt8] {
        ...
        for chunkStart in stride(from: 0, to: message.count, by: 64) {
            ...
            for i in 0..<64 {
                ...
                d = c
                c = b
                b = b &+ ((f << md5Shift[i]) | (f >> (32 - md5Shift[i])))
            }
            a0 &+= a
            b0 &+= b
            c0 &+= c
    ...
    static func sha1(of input: [UInt8]) -> [UInt8] {
